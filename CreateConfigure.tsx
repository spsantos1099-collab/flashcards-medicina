    ...
    return (
      <div className="max-w-xl">
        ...
        <p className="text-ink-400 mb-6">
          O arquivo fica disponível somente durante esta criação. Se a página foi atualizada, selecione o PDF ou DOCX novamente.
        </p>
        <Link
    ...
    <div className="max-w-xl">
      ...
      <div className="rounded-card border border-clinical-300 dark:border-clinical-700/60 bg-clinical-50/60 dark:bg-clinical-700/10 px-4 py-4 mb-4">
        <div className="flex items-start justify-between gap-4">
          <div>
            <div className="source-tab text-clinical-700 dark:text-clinical-200">CONTEÚDO PRONTO</div>
        ...
        <p className="text-sm text-ink-500 dark:text-ink-200 mt-3">
          O conteúdo foi preparado para criar os cards. O arquivo original não fica salvo na sua conta.
        </p>
      </div>
      ...
      {extractedDocument.warnings.length > 0 && (
        <div className="rounded-card border border-amber-300/70 bg-amber-50/70 dark:bg-amber-950/20 dark:border-amber-800 px-4 py-3 mb-4">
          <div className="source-tab text-amber-800 dark:text-amber-300">VERIFIQUE O CONTEÚDO</div>
      ...
      <div className="rounded-card border border-ink-200/70 dark:border-ink-800 mb-7 overflow-hidden">
        ...
        >
          <div>
            <div className="source-tab">PRÉVIA DO CONTEÚDO</div>
            <div className="text-sm font-medium text-ink-800 dark:text-paper mt-1">
              {showPreview ? "Ocultar amostra" : "Ver trecho do documento"}
            </div>
          </div>
      ...
      <div className="rounded-card border border-ink-200/70 dark:border-ink-800 px-4 py-3.5 mb-7">
        <div className="source-tab text-clinical-600 dark:text-clinical-300">REFERÊNCIAS</div>
        <p className="text-sm text-ink-400 mt-1">
          {extractedDocument.extension === "pdf"
            ? "Os cards podem indicar a página e o trecho do PDF que deram origem ao conteúdo."
            : "Os cards citarão o documento e o trecho usado como referência."}
        </p>
      </div>
      ...
      <div className="rounded-card border border-clinical-300 dark:border-clinical-700/60 bg-clinical-50/50 dark:bg-clinical-700/10 px-4 py-4">
        <div className="source-tab text-clinical-700 dark:text-clinical-200">PRONTO PARA GERAR</div>
      </div>
      ...
      {generating && generationProgress && (
        <div className="mt-4 rounded-lg border border-ink-200/70 dark:border-ink-800 bg-white/60 dark:bg-ink-950/30 px-3.5 py-3 text-sm">
            ...
            <span className="font-medium text-ink-800 dark:text-paper">
              {generationProgress.label
                ...
                : generationProgress.stage === "retrying"
                  ? `Tentando novamente · tentativa ${generationProgress.retryAttempt ?? 2}`
          ...
          <p className="text-xs text-ink-400 mt-2">Você pode acompanhar o progresso enquanto os cards são preparados.</p>
        </div>
      )}
