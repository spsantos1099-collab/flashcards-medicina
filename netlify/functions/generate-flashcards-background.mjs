...
function normalizeJobPayload(body) {
  ...
  const cardTypes = Array.isArray(options.cardTypes)
    ...
    : [];
  const cardCount = Math.max(1, Math.min(40, Number(options.cardCount) || 15));
  return {
...
function sourceInput(payload, type, cardCount, accepted, generationPhase = "initial") {
  return {
    ...
    options: {
      ...
      cardCount,
      generationPhase,
      excludedQuestions: [...(payload.options.excludedQuestions || []), ...accepted.map((card) => card.question)].slice(-40),
      excludedObjectives: [...(payload.options.excludedObjectives || []), ...accepted.map((card) => card.learningObjective).filter(Boolean)].slice(-40),
    },
    candidates: [],
  };
}
